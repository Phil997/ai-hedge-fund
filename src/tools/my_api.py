...
def search_line_items(
    ...
    url = "financials/search/line-items"
...
def get_insider_trades(
    ...
    while True:
        url = f"insider-trades/?ticker={ticker}&filing_date_lte={current_end_date}"
        if start_date:
...
def get_company_news(
    ...
    while True:
        url = f"news/?ticker={ticker}&end_date={current_end_date}"
        if start_date:
...
def get_market_cap(
    ...
    if end_date == datetime.datetime.now().strftime("%Y-%m-%d"):
        ...
        url = f"company/facts/?ticker={ticker}"
        response = _make_api_request(url, headers)
        if response.status_code != 200:
