...
def get_available_models(ollama_url: str) -> list:
    ...
    try:
        ...
        print(
            f"{Fore.RED}Failed to get available models from Ollama service."
...
def download_model(model_name: str, ollama_url: str) -> bool:
    ...
    while total_wait_time < max_wait_time:
        ...
        if total_wait_time % 60 == 0:
            ...
            print(
                f"{Fore.CYAN}Download in progress... ({minutes}"
                f" minute{'s' if minutes != 1 else ''}"
                f" elapsed){Style.RESET_ALL}"
            )

    # If we get here, we've timed out
    print(
        f"{Fore.RED}Timed out waiting for model download to complete"
        f" after {max_wait_time // 60} minutes.{Style.RESET_ALL}"
    )
    return False
