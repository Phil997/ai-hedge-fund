...
def start_ollama_server() -> bool:
    ...
    try:
        ...
        print(
            f"{Fore.RED}Failed to start Ollama server. Timed out waiting"
            f" for server to become available.{Style.RESET_ALL}"
        )
        return False
...
def install_ollama() -> bool:
    ...
    if system not in OLLAMA_DOWNLOAD_URL:
        print(
            f"{Fore.RED}Unsupported operating system for automatic"
            f" installation: {system}{Style.RESET_ALL}"
        )
    ...
    if system == "darwin":  # macOS
        ...
        if questionary.confirm(
            "Would you like to download the Ollama application?",
            default=True,
        ).ask():
            try:
                import webbrowser
                ...
                print(
                    f"{Fore.YELLOW}Please download and install the"
                    f" application, then restart this"
                    f" program.{Style.RESET_ALL}"
                )
                print(
                    f"{Fore.CYAN}After installation, you may need to open"
                    f" the Ollama app once before"
                    f" continuing.{Style.RESET_ALL}"
                )

                # Ask if they want to try continuing after installation
                if questionary.confirm(
                    "Have you installed the Ollama app and opened it"
                    " at least once?",
                    default=False,
                ).ask():
                    ...
                    if is_ollama_installed() and start_ollama_server():
                        print(
                            f"{Fore.GREEN}Ollama is now properly installed"
                            f" and running!{Style.RESET_ALL}"
                        )
                        return True
                    else:
                        print(
                            f"{Fore.RED}Ollama installation not detected."
                            f" Please restart this application after"
                            f" installing Ollama.{Style.RESET_ALL}"
                        )
                        return False
                return False
            except Exception as e:
            ...
            if questionary.confirm(
                "Would you like to try the command-line installation"
                ...
                try:
                    ...
                    if install_process.returncode == 0:
                        print(
                            f"{Fore.GREEN}Ollama installed successfully"
                            f" via command line.{Style.RESET_ALL}"
                        )
                        return True
                    else:
                        print(
                            f"{Fore.RED}Command-line installation failed."
                            f" Please use the app download method"
                            f" instead.{Style.RESET_ALL}"
                        )
                        return False
                except Exception as e:
        ...
        try:
            ...
            if install_process.returncode == 0:
                ...
                print(
                    f"{Fore.RED}Failed to install Ollama."
                    f" Error: {install_process.stderr}{Style.RESET_ALL}"
                )
                return False
        except Exception as e:
...
def download_model(model_name: str) -> bool:
    ...
    try:
            ...
            if output:
                ...
                if percentage is not None:
                    ...
                    if abs(percentage - last_percentage) >= 1 or (current_phase and current_phase != last_phase):
                        ...
                        phase_display = (
                            f"{Fore.CYAN}{last_phase.capitalize()}"
                            f"{Style.RESET_ALL}: "
                            if last_phase else ""
                        )
                        status_line = (
                            f"\r{phase_display}{Fore.GREEN}{bar}"
                            f"{Style.RESET_ALL}"
                            f" {Fore.YELLOW}{percentage:.1f}%"
                            f"{Style.RESET_ALL}"
                        )
        ...
        if return_code == 0:
            ...
            print(
                f"{Fore.RED}Failed to download model {model_name}."
                f" Check your internet connection and"
                f" try again.{Style.RESET_ALL}"
            )
            return False
    except Exception as e:
...
def ensure_ollama_and_model(model_name: str) -> bool:
    ...
    if (
        env_override
        or ollama_url.startswith("http://ollama:")
        or ollama_url.startswith("http://host.docker.internal:")
    ):
    ...
    if model_name not in available_models:
        ...
        if questionary.confirm(
            f"Do you want to download the {model_name}"
            f" model?{model_size_info}"
...
def delete_model(model_name: str) -> bool:
    ...
    in_docker = (
        os.environ.get("OLLAMA_BASE_URL", "").startswith("http://ollama:")
