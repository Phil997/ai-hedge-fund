...
def analyze_moat_strength(metrics: list, financial_line_items: list) -> dict:
    ...
    if len(financial_line_items) >= 3:
        ...
        for item in financial_line_items:
            if (hasattr(item, 'capital_expenditure') and item.capital_expenditure is not None
                    and hasattr(item, 'revenue') and item.revenue is not None and item.revenue > 0):
    ...
    goodwill_and_intangible_assets = [
        item.goodwill_and_intangible_assets for item in financial_line_items
...
def analyze_management_quality(financial_line_items: list, insider_trades: list) -> dict:
    ...
    cash_values = [
        item.cash_and_equivalents for item in financial_line_items
...
def analyze_predictability(financial_line_items: list) -> dict:
    ...
    if op_income and len(op_income) >= 5:
        ...
        if positive_periods == len(op_income):
            ...
            score += 1
            details.append(
                f"Somewhat predictable operations: Operating income positive"
                f" in {positive_periods}/{len(op_income)} periods"
            )
        else:
            details.append(
                f"Unpredictable operations: Operating income positive"
                f" in only {positive_periods}/{len(op_income)} periods"
            )
    else:
    ...
    if fcf_values and len(fcf_values) >= 5:
        ...
        if positive_fcf_periods == len(fcf_values):
            ...
            details.append(
                f"Unpredictable cash generation: Positive FCF"
                f" in only {positive_fcf_periods}/{len(fcf_values)} periods"
            )
    else:
