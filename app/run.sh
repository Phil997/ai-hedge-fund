    ...
    if ! command_exists uv; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            if python3 -m pip install uv; then
                ...
                print_error "Please install UV manually from https://github.com/astral-sh/uv"
                exit 1
            fi
        else
            missing_deps+=("UV (https://github.com/astral-sh/uv)")
        fi
    fi
...
install_backend() {
    ...
    if uv run python -c "import uvicorn; import fastapi" >/dev/null 2>&1; then
        ...
        if uv run python -c "import uvicorn; import fastapi" >/dev/null 2>&1; then
            ...
            print_error "Try running: cd backend && uv sync"
            exit 1
        fi
    fi
