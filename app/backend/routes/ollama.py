...
async def start_ollama_server():
    ...
    try:
        ...
        if not result["success"]:
            logger.error(
                f"Failed to start Ollama server: {result['message']}"
            )
            raise HTTPException(
...
async def stop_ollama_server():
    ...
    try:
        ...
        if not result["success"]:
            logger.error(
                f"Failed to stop Ollama server: {result['message']}"
            )
            raise HTTPException(
                status_code=500, detail=result["message"]
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to stop Ollama server: {str(e)}"
        )
...
async def download_model(request: ModelRequest):
    ...
    try:
        ...
        status = await ollama_service.check_ollama_status()
        logger.debug(
            f"Current Ollama status: installed={status['installed']}, "
        ...
        if not result["success"]:
            logger.error(
                f"Failed to download model {request.model_name}: "
                f"{result['message']}"
            )
            raise HTTPException(
                status_code=500, detail=result["message"]
        ...
        logger.info(
            f"Successfully downloaded model: {request.model_name}"
        )
        return ActionResponse(**result)
    except HTTPException:
        raise
        ...
        logger.error(
            f"Unexpected error downloading model "
            f"{request.model_name}: {e}"
        )
        raise HTTPException(
            status_code=500,
...
async def download_model_with_progress(request: ModelRequest):
    """Download an Ollama model with real-time progress updates via SSE."""
    try:
        logger.info(
            f"Progress download request for model: {request.model_name}"
        )
        ...
        logger.error(
            f"Unexpected error setting up progress download for "
            f"{request.model_name}: {e}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"Failed to start progress download: {str(e)}"
        )
...
async def get_download_progress(model_name: str):
    ...
    try:
        ...
        if progress is None:
            raise HTTPException(
                status_code=404,
                detail=f"No active download found for model: {model_name}"
            )
        ...
        logger.error(
            f"Error getting download progress for {model_name}: {e}"
        )
        raise HTTPException(
            status_code=500,
...
async def get_active_downloads():
    ...
    try:
        ...
        # Only return downloads that are actually active
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get active downloads: {str(e)}"
        )
...
async def delete_model(model_name: str):
    ...
    try:
        ...
        if not result["success"]:
            logger.error(
                f"Failed to delete model {model_name}: {result['message']}"
            )
            raise HTTPException(
                status_code=500, detail=result["message"]
            )
        ...
        logger.error(
            f"Unexpected error deleting model {model_name}: {e}"
        )
        raise HTTPException(
            status_code=500,
...
async def cancel_download(model_name: str):
    ...
    try:
        ...
        logger.error(
            f"Unexpected error cancelling download for {model_name}: {e}"
        )
        raise HTTPException(
            status_code=500,
